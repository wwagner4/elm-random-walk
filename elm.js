...
Elm.RandomWalkModel = Elm.RandomWalkModel || {};
Elm.RandomWalkModel.make = function (_elm) {
   "use strict";
   _elm.RandomWalkModel = _elm.RandomWalkModel || {};
   if (_elm.RandomWalkModel.values) return _elm.RandomWalkModel.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   var diffVal = 5.0;
   var diffGen = A2($Random.$float,0 - diffVal,diffVal);
   var ranDiff = function (seed) {    return A2($Random.generate,diffGen,seed);};
   var updateX = F3(function (panel,seed,pos) {
      var max = panel.w / 2.0;
      var corr1 = _U.cmp(pos.x,max * 0.9) > 0 ? 0 - diffVal : 0;
      var corr2 = _U.cmp(pos.x,(0 - max) * 0.9) < 0 ? diffVal : 0;
      var _p0 = ranDiff(seed);
      var diff = _p0._0;
      var nextSeed = _p0._1;
      var nextX = pos.x + diff + corr1 + corr2;
      return {ctor: "_Tuple2",_0: nextX,_1: nextSeed};
   });
   var updateY = F3(function (panel,seed,pos) {
      var max = panel.h / 2.0;
      var corr1 = _U.cmp(pos.y,max * 0.9) > 0 ? 0 - diffVal : 0;
      var corr2 = _U.cmp(pos.y,(0 - max) * 0.9) < 0 ? diffVal : 0;
      var _p1 = ranDiff(seed);
      var diff = _p1._0;
      var nextSeed = _p1._1;
      var nextY = pos.y + diff + corr1 + corr2;
      return {ctor: "_Tuple2",_0: nextY,_1: nextSeed};
   });
   var updatePos = F3(function (panel,seed,pos) {
      var _p2 = A3(updateX,panel,seed,pos);
      var nextX = _p2._0;
      var s1 = _p2._1;
      var _p3 = A3(updateY,panel,s1,pos);
      var nextY = _p3._0;
      var s2 = _p3._1;
      var nextPos = _U.update(pos,{x: nextX,y: nextY});
      return {ctor: "_Tuple2",_0: nextPos,_1: s2};
   });
   var updateElem = F3(function (panel,seed,elem) {
      var _p4 = A3(updatePos,panel,seed,elem.pos);
      var nextPos = _p4._0;
      var nextSeed = _p4._1;
      var nextElem = _U.update(elem,{pos: nextPos});
      return {ctor: "_Tuple2",_0: nextElem,_1: nextSeed};
   });
   var updateFoldElem = F2(function (elem,_p5) {
      var _p6 = _p5;
      var _p8 = _p6._2;
      var _p7 = A3(updateElem,_p8,_p6._1,elem);
      var nextElem = _p7._0;
      var nextSeed = _p7._1;
      var nextElems = A2($List._op["::"],nextElem,_p6._0);
      return {ctor: "_Tuple3",_0: nextElems,_1: nextSeed,_2: _p8};
   });
   var update = F2(function (panelDim,model) {
      var _p9 = A3($List.foldl,updateFoldElem,{ctor: "_Tuple3",_0: _U.list([]),_1: model.seed,_2: panelDim},model.elems);
      var nextElems = _p9._0;
      var nextSeed = _p9._1;
      var panelDim = _p9._2;
      return _U.update(model,{elems: nextElems,seed: nextSeed});
   });
   var initialPos = {x: 0.0,y: 0.0};
   var initialElem = {pos: initialPos};
   var initial = {seed: $Random.initialSeed(821736182376),elems: A2($List.repeat,100,initialElem)};
   var Model = F2(function (a,b) {    return {seed: a,elems: b};});
   var Elem = function (a) {    return {pos: a};};
   var Pos = F2(function (a,b) {    return {x: a,y: b};});
   var PanelDim = F2(function (a,b) {    return {w: a,h: b};});
   return _elm.RandomWalkModel.values = {_op: _op
                                        ,PanelDim: PanelDim
                                        ,Pos: Pos
                                        ,Elem: Elem
                                        ,Model: Model
...
Elm.RandomWalkView = Elm.RandomWalkView || {};
Elm.RandomWalkView.make = function (_elm) {
   "use strict";
   _elm.RandomWalkView = _elm.RandomWalkView || {};
   if (_elm.RandomWalkView.values) return _elm.RandomWalkView.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $RandomWalkModel = Elm.RandomWalkModel.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var form = function (shape) {    return A2($Graphics$Collage.alpha,0.5,A2($Graphics$Collage.filled,$Color.green,shape));};
   var shape = $Graphics$Collage.circle(70.0);
   var toForm = function (elem) {    var y = elem.pos.y;var x = elem.pos.x;return A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: x,_1: y},form(shape));};
   var view = F2(function (panel,model) {
      var h = $Basics.round(panel.h);
      var w = $Basics.round(panel.w);
      return A3($Graphics$Collage.collage,w,h,A2($List.map,toForm,model.elems));
   });
   return _elm.RandomWalkView.values = {_op: _op,shape: shape,form: form,toForm: toForm,view: view};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $RandomWalkModel = Elm.RandomWalkModel.make(_elm),
   $RandomWalkView = Elm.RandomWalkView.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Time = Elm.Time.make(_elm),
   $Window = Elm.Window.make(_elm);
   var _op = {};
   var toPanelDim = function (_p0) {    var _p1 = _p0;return {w: $Basics.toFloat(_p1._0),h: $Basics.toFloat(_p1._1)};};
   var panelDim = function (dimensions) {    return A2($Signal.map,toPanelDim,dimensions);};
   var time = $Time.every($Time.millisecond * 10);
   var panelDimTriggered = A2($Signal.sampleOn,time,panelDim($Window.dimensions));
   var main = A3($Signal.map2,
   $RandomWalkView.view,
   panelDim($Window.dimensions),
   A3($Signal.foldp,$RandomWalkModel.update,$RandomWalkModel.initial,panelDimTriggered));
   return _elm.Main.values = {_op: _op,time: time,toPanelDim: toPanelDim,panelDim: panelDim,panelDimTriggered: panelDimTriggered,main: main};
};
